...
class data_class(object):
    ...
    def load_chunk(self):
        ...
        for f in self.file_names[self.chunk_cursor:self.chunk_cursor + self.chunk_size]:
            ...
            #pridavam prvok nahodneho zuzenia na 70-100 percent
            wpercent = self.image_height / float(im.size[1])
            if self.data_use == "train": wpercent = wpercent * (np.random.randint(30)+71)/100
            wsize = int((float(im.size[0]) * float(wpercent)))
            im = np.array(im.resize((wsize, self.image_height))).astype(float) / 255

            #nahodna inverzia, erosion and dilation
            if self.data_use == "train":
                r = np.random.randint(3)
                k = np.random.randint(3) + 2
                if r == 0:
                    im = cv2.erode(im, np.ones((k, k), np.uint8), iterations=1)
                elif r == 1:
                    im = cv2.dilate(im, np.ones((k, k), np.uint8), iterations=1)

                subset = np.random.choice(np.array([0, 1, 2]))
                im[:, :, subset] = 1 - im[:, :, subset]
    ...
    def next_batch(self):
        ...
        #ak z nejakeho dovodu nie je dost dat do  batchu tak to sposobi errory
...
with graph.as_default():
    ...
    weights = {
        ...
        'out': tf.Variable(tf.truncated_normal(
            [num_hidden_lstm[1], nr_classes], stddev=np.sqrt(2 / (num_hidden_lstm[1]))))
    }
    ...
    with tf.variable_scope('lstm1'):
        lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(num_hidden_lstm[0], state_is_tuple=True)
    with tf.variable_scope('lstm2'):
        lstm_cell_2 = tf.nn.rnn_cell.BasicLSTMCell(num_hidden_lstm[1], state_is_tuple=True)
    ...
    def model(data):
        ...
        print('LSTM1:', out.get_shape().as_list())

        with tf.variable_scope('lstm2'):
            out, state_lstm_2 = tf.nn.dynamic_rnn(
                cell=lstm_cell_2, inputs=out, dtype=tf.float32, sequence_length=tf_seq_len)

        print('LSTM2:', out.get_shape().as_list())
...
with tf.Session(graph=graph) as session:
    for ensemble_num in range(5):
        if os.path.isfile("abcdefghij/{}_ens{}.ckpt".format(session_log_name, ensemble_num)):
            saver.restore(session, "abcdefghij/{}_ens{}.ckpt".format(session_log_name, ensemble_num))
        else:
            session.run(initialization)


        print('------------------------')
        print('Training {}_ens{}'.format(session_log_name, ensemble_num))
        print('------------------------')
