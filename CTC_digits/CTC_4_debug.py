...
class data_class(object):
    """
    POUZITIE
    -vytvor instanciu meno = data_class(args)
    ...
    def __init__(self, path, batch_size, chunk_size, image_height, cut_width, stride, max_frames, data_use="train"):
        self.data = None
        self.labels = None
        self.seq_len = None
        ...
        self.file_names = self.load_data()
        ...
        self.next_chunk()
        # chcem ukladat uz resized? zatial nie

    def load_data(self):
        all_img = os.listdir(self.path)
        separator = (len(all_img) * 4) // 5
        if self.data_use == "test":
            names = all_img[separator:]
            self.size = len(names)
        else:
            names = all_img[:separator]
            self.size = len(names)
    ...
    def load_chunk(self):
        ...
        for f in self.file_names[self.chunk_cursor:self.chunk_cursor + self.chunk_size]:
            ...
            wpercent = self.image_height / float(im.size[1])
            wsize = int((float(im.size[0]) * float(wpercent)))
            im = np.array(im.resize((wsize, self.image_height))).astype(float) / 255
            chunk_imgs.append(im)

            target_str = ''.join(f.split('.', 1))[:-4].split('_')[-1]
            chunk_labels.append(target_str)
        ...
        return chunk_imgs, chunk_labels

    def cut_and_pad(self, image_data, num_frames, cut_width, stride):
        img_width = np.shape(image_data)[1]
        ...
        # maxi? to bude musiet byt
        # a) bud konstanta
        # b) alebo to vyratam na uvod
    ...
    def next_chunk(self):
        ...
        for img in self.data:
            a, b = self.cut_and_pad(img, self.max_frames, self.cut_width, self.stride)
            data_vyrezy.append(a)
            seq_len.append(b)

        self.data = np.array(data_vyrezy)
        self.seq_len = np.array(seq_len)
        print('Got it')
    ...
    def next_batch(self):
        ...
        seq_len = self.seq_len[self.batch_cursor:self.batch_cursor + self.batch_size]

        self.batch_cursor += self.batch_size
        if self.batch_cursor + self.batch_size > self.chunk_size:
            self.batch_cursor = 0
            self.next_chunk()
...
###############################
conv_layer_names = ['conv1','conv1_5']#, 'conv2']#, 'conv3', 'conv4']
...
# DROPOUT
keep_prob_conv = np.ones_like(conv_layer_names)#np.array([0.9, 0.8, 0.7, 0.6])
...
with graph.as_default():
    ...
    weights = {
        ...
        'fc3': tf.Variable(tf.truncated_normal(
            [num_hidden[1],
             num_hidden[2]],
            stddev=np.sqrt(2 / (num_hidden[1])))
        ),

        'fc4': tf.Variable(tf.truncated_normal(
            [num_hidden[2],
             num_hidden[3]],
            stddev=np.sqrt(2 / (num_hidden[2])))
        ),
    ...
    i = 1
    l = conv_layer_names[i]
    out2 = tf.nn.conv3d(poconv1, weights[l], [1, 1, strides[l][0], strides[l][1], 1], padding=paddings[l])
    # out = tf.nn.dropout(out, tf_keep_prob_conv[i])
    out3 = tf.nn.relu(out2 + biases[l])
    ...
    out = tf.einsum('ijk,lk->ijl', out, tf.transpose(weights['fc3'])) + biases['fc3']
    out = tf.nn.dropout(out, tf_keep_prob_fc)
    out = tf.nn.relu(out)

    print('fc3:', out.get_shape().as_list())

    out = tf.einsum('ijk,lk->ijl', out, tf.transpose(weights['fc4'])) + biases['fc4']
    out = tf.nn.dropout(out, tf_keep_prob_fc)
    out = tf.nn.relu(out)

    print('fc4:', out.get_shape().as_list())
    ...
    output = [tf.matmul(out[:, i, :], weights['out']) + biases['out'] for i in range(maxi)]
...
with tf.Session(graph=graph) as session:
    ...
    while pokracovat == 1:
        ...
        print('ideme na to')
        session.run(
            [predconv], feed_dict=feed_dict)
        print('ide konvolucia')
        session.run(
            [poconv1], feed_dict=feed_dict)
        print('Konvolucia1 uspesna')
        '''session.run(
            [pocasconv2], feed_dict=feed_dict)
        print('Konvolucia2 v polke')
        session.run(
            [poconv2], feed_dict=feed_dict)
        print('Konvolucia2 uspesna')
        session.run(
            [poconv3], feed_dict=feed_dict)
        print('Konvolucia3 uspesna')
        session.run(
            [poconv4], feed_dict=feed_dict)
        print('Konvolucia4 uspesna')'''
        session.run(
            [pofc], feed_dict=feed_dict)
        print('FC uspesna')
        session.run(
            [polstm], feed_dict=feed_dict)
        print('lstm uspesna')
        session.run(
            [output], feed_dict=feed_dict)
        print('output uspesny')
        print('Going to step')
        ...
        if (step % info_freq == 0):
            ...
            print('Test accuraccy (batch-sized subset)',
                  100 * accuracy(test_predictions, batch_labels_test, test_seq_len) / batch_size)
            print('------------------------')

        if (step % num_steps) == 0:
            pokracovat = int(input('Continue? 1/0'))
